...
Apart from `--run-interpreter`, and whatever `claude mcp get` starts in order to answer for the MCP channel, nothing a project holds can take the report away from `doctor`: it always finishes, printing one line per channel — and its fix — that it wrote itself. That matters because a project can arrive with `git clone`: git carries symlinks, and `git add -f` puts even a git-ignored `settings.local.json` in a commit. A matcher is checked and evaluated in time proportional to its length. A registered path the OS will not look up reads as `degraded`, since Claude Code meets the same wall. A path `doctor` cannot look up — an unreadable directory, a symlink loop — reads as `unknown`, and so does a settings file it can open but not finish parsing (bytes that are not UTF-8, an integer too long to convert, nesting too deep, `NaN` or `Infinity`) or one over 1 MiB. Every string printed from a settings file, a link, or `claude mcp get` has its control characters escaped (`\x0a`), and output to a stream whose encoding is not UTF-8 is written as ASCII, so none of them can add a line. `claude` is looked up, and run, with the relative `PATH` entries dropped — the ones the current directory decides, such as `node_modules/.bin` — since `claude` is often a `#!/usr/bin/env node` script and a project could otherwise supply either it or its interpreter; a `claude` found only through such an entry is not run, and the MCP channel reads `unknown`.
...
**The server it starts may be the project's, not yours.** A project's `.mcp.json` can register a server under mpg's name, and a project-scope registration shadows the user one. In a folder you have trusted in Claude Code, if the project's own `.claude/settings.json` or `.claude/settings.local.json` approves its `.mcp.json` servers — and both can arrive with a clone — `claude mcp get` starts that server to answer, so `mpg doctor` runs the command it names, as you. In a folder not yet trusted, or without that approval, the server is reported as pending approval and not started (measured with Claude Code 2.1.268). This is Claude Code's behaviour — opening `claude` in the same folder starts the same server — and `doctor` inherits it rather than adding to it.
...
- Non-Python files, paths with nothing at them (or nothing that is a regular file), input without a `.py` `tool_input.file_path`, and clean files produce no output.
- **A file the hook could not check produces a one-line warning; a clean file still produces nothing.** "Could not check" means the check did not run to completion on the edited file: the file cannot be reached or read, is over 2 MiB, is binary, the bundled guide catalog loads no guides, or a detector raises. The hook then returns a `systemMessage` naming the reason, which Claude Code shows to you and not to Claude; the edit is never blocked and the exit status stays 0. It says so on every edit, since each edit is a new check, and there is no per-file switch — `mpg setup --no-hook` removes the hook altogether. When the file itself is the reason (too large, unreadable, binary), `mpg check <file>` reports the same reason on stderr with exit 2; the exact text can differ, since the hook shows control characters as escapes and `check` does not. Silence still means the file was checked and nothing was found. A project config the hook cannot parse is a different case: the check still runs, against the default target, and the `source: default` in a findings line is the only trace.
